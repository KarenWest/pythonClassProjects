...
import string
from unused_node_removed import Edge, WeightedDigraph
from decorators_for_timing_memoize_and_tracing import timing

LARGE_DIST = 1000000

def total_distances(graph, path, total=None, outdoors=None):
    """ Find total distance traveled and outdoors traveled,
        if it is less than total and outdoors parametersif those are given 
    """
    distances = [(0, 0)]
    for this, after in zip(path, path[1:]):
        ...
        for edge in w:
            ...
            new_distances.extend([(tot + this_total, ins + this_outdoors) for tot, ins in distances
...
def bruteForceSearch(digraph, start, end, maxTotalDist, maxDistOutdoors):
    ...
    while new_path:
        path = new_path
        new_path = [p + [child] for p in path for child in digraph.childrenOf(p[-1]) if child not in p and p[-1] != end]
        new_solutions = [(total_distances(digraph, p, maxTotalDist, maxDistOutdoors), p) for p in new_path if p[-1] == end]
        if new_path:
...
def directedBFS(digraph, start, end, maxTotalDist, maxDistOutdoors):
    ...
    while new_path:
        path = new_path
        new_path = [((total + total_child, outdoors + outdoors_child), p + [child]) for (total, outdoors), p in path
                            for child in digraph.childrenOf(p[-1])
                            for total_child, outdoors_child  in digraph.weights[p[-1], child]
                            if child not in p and child != end and (total + total_child <= maxTotalDist and outdoors + outdoors_child <= maxDistOutdoors)]
        solutions.extend([((total + total_child, outdoors + outdoors_child), p + [child]) for (total, outdoors), p in path
                            for child in digraph.childrenOf(p[-1])
                            for total_child, outdoors_child  in digraph.weights[p[-1], child]
                            if child == end and (total + total_child <= maxTotalDist and outdoors + outdoors_child <= maxDistOutdoors)])
        print len(solutions),
    print
    if not solutions:
        raise ValueError
