...
class Patient(object):
    ...
    def update(self):
        ...
        density = len(self.viruses) / float(self.max_pop)
        temp_viruses = self.viruses[:]
        for v in temp_viruses:
            try:
                virus = v.reproduce(density)
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        """
        return sum(v.isResistantTo(d) for d in drugResist 
    ...
    def update(self):
        ...
        """
        self.viruses = [v for v in self.viruses if not v.doesClear()]
        density = len(self.viruses) / float(self.getMaxPop())
        temp_viruses = self.viruses[:]
        for v in temp_viruses:
            try:
                virus = v.reproduce(density, self.drugs)
                self.viruses.append(virus)
            except NoChildException:
                pass
        return len(self.viruses)
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    steps: number of time steps to run simulation total
    stage: steps to pass before adminstering the drug
    """
    #stage = 150 (for PS9, passed as a param)
    #steps = 300 (for PS9, passed as a param)
    virusPopEachStepList = [0.0] * steps
    #virusPopRepOnceList = []
    #virusPopEachTrialList = [0.0] * numTrials
    virusResistPopEachStepList = [0.0] * steps
    for t in range(numTrials):
        ...
        for step in range(steps):
            ...
            virusPopEachStepList[step] += tp.update()
            #print "virus pop " + str(virusPopEachStepList[step]) + " step " + str(step)
            virusResistPopEachStepList[step] += tp.getResistPop(['guttagonol'])
        
        #for step in range(steps):
            #if (int(virusPopEachStepList[step])) not in virusPopRepOnceList:
                #virusPopRepOnceList.append(int(virusPopEachStepList[step]))
                #print "virus pop " + str(virusPopRepOnceList[-1])
            
        #print "trial " + str(t) + " virus pop total all steps " + str(virusPopEachTrialList[t]) + " for total steps " + str(steps)
        #virusPopEachTrialList[t] = int(virusPopEachTrialList[t] / steps)
        #print "virus pop when divided by total steps " + str(virusPopEachTrialList)
    #return virusPopEachStepList 
        #print('Trial: ' + str(t))
    #for i in range(steps):
        #virusPopEachStepList[i] /= numTrials
        #virusResistPopEachStepList[i] /= numTrials
    return virusPopEachStepList
...
def simulationWith2Drugs(numViruses, maxPop, maxBirthProb, clearProb, resistances,
                       mutProb, numTrials, steps, stage1, stage2):
    """
    Runs simulations and plots graphs for problem 5.
    ...
    steps: number of time steps to run simulation total
    stage1: steps to pass before adminstering the drug1
    stage2: steps to pass before adminstering the drug2
    """
    #stage = 150 (for PS9, passed as a param)
    #steps = 300 (for PS9, passed as a param)
    virusPopEachStepList = [0.0] * steps
    ...
    for t in range(numTrials):
        ...
        for step in range(steps):
            ...
            virusPopEachStepList[step] += tp.update()
            #print "virus pop " + str(virusPopEachStepList[step]) + " step " + str(step)
            virusResistPopEachStepList[step] += tp.getResistPop(['guttagonol','grimpex'])
